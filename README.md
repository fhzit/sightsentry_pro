...
WiFi 设备来自 802.11 Probe Request。新版固件输出类型为 `WIFI_PROBE`；新版客户端同时兼容旧固件的 `WIFI` 和新版固件的 `WIFI_PROBE`，并使用 Probe Request 的源 MAC 做 IEEE OUI 厂商识别。

示例：

```
1|AA:BB:CC:DD:EE:FF|-65|WIFI_PROBE|
1|11:22:33:44:55:66|-72|BLE|AirPods
```
