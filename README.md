...
新版固件会在末尾追加 BLE 广播元数据字段：

```
节点编号|MAC地址|RSSI|信号类型|名称|ManufacturerDataHex|ServiceUUIDs|TxPower|Appearance
```

WiFi 设备来自 802.11 Probe Request。新版固件输出类型为 `WIFI_PROBE`；新版客户端同时兼容旧固件的 `WIFI` 和新版固件的 `WIFI_PROBE`，并使用 Probe Request 的源 MAC 做 IEEE OUI 厂商识别。BLE 设备优先使用广播名称，其次根据 Manufacturer Data 的 Bluetooth Company Identifier 推断厂商，最后回退到 MAC OUI。

WiFi 探测和 BLE 探测在客户端内按 `信号类型 + MAC` 分开建档、分开展示，不会把 WiFi Probe 与 BLE 广播融合成同一个设备。
